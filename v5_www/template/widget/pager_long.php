...
$currentPage = U('Http')->getGET('page', '1');
...
$firstUrl = ($firstPage && $currentPage!=1) ? U('Url')->getCurrentUrl(array('page' => 1)) : '#';
$lastUrl  = ($lastPage && $currentPage!=$lastPage) ? U('Url')->getCurrentUrl(array('page' => $lastPage)) : '#';
$preUrl   = ($prePage && $prePage>=1) ? U('Url')->getCurrentUrl(array('page' => $currentPage - 1)) : '#';
$nextUrl  = ($nextPage && $nextPage<=$lastPage) ? U('Url')->getCurrentUrl(array('page' => $currentPage + 1)) : '#';
...
<ul class="widget-pager">
    ...
    if ($lastPage) {
        for ($i = $beginPage; $i < $beginPage+$length; $i ++) {
            ...
            $url = ($currentPage == $i) ? '#' : U('Url')->getCurrentUrl(array('page' => $i));
            echo sprintf('<li><a class="item item-lg %s" name="pager" value="%s" href="%s">%s</a></li>', $isSelected, $i, $url, $i);
        }
    }
